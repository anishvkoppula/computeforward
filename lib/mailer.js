  ...
  return `<!doctype html><html><body style="margin:0;background:#f3efe5;color:#10243d;font-family:Georgia,serif">
    <div style="max-width:640px;margin:0 auto;padding:40px 24px">
      <div style="border-top:6px solid #ff5c35;background:#fff;padding:32px;border-radius:0 0 14px 14px">
        ...
        ${body}
        <p style="margin:30px 0 0;color:#526271;font-size:14px;line-height:1.6">Questions? Reply to this email or contact anishkoppula@gmail.com and kaushik.atla@gmail.com.</p>
      </div>
    </div></body></html>`;
}
...
export function createMailer(config, dependencies = {}) {
  ...
  return {
    ...
    async sendApplicationConfirmation(application) {
      ...
      const recipients = [...new Set([applicant.email, applicant.guardianEmail].filter(Boolean).map(value => value.toLowerCase()))];
      const textSummary = [
